...
const API_CACHE_CONFIG = {
    ...
    '/api/attractions': { strategy: 'staleWhileRevalidate', maxAge: 60 * 60 * 1000 }, // 景点数据1小时
    '/api/restaurants': { strategy: 'staleWhileRevalidate', maxAge: 60 * 60 * 1000 }, // 餐厅数据1小时
...
async function handleNavigationRequest(request) {
    try {
        ...
        // 最后的回退
        return new Response(
            '<!DOCTYPE html><html><head><title>离线</title></head><body><h1>当前离线</h1><p>请检查网络连接</p></body></html>', { headers: { 'Content-Type': 'text/html; charset=utf-8' } }
        );
    }
}
...
self.addEventListener('push', event => {
    ...
    event.waitUntil(
        self.registration.showNotification('GoingTour', options)
    );
});
