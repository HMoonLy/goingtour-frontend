...
export const weatherApi = {
    ...
    generateWeatherSuggestions(liveWeather, forecastWeather, startDate, days) {
        ...
        console.log(`✅ 最终构造的天气预报数据共${forecastList.length}条:`, forecastList);
    ...
    async generateHistoricalWeatherSuggestions(city, startDate, days) {
        try {
            ...
            // 基于历史数据生成建议
            return this.buildSuggestionsFromHistoricalData(historicalData, city, startDate, days);
        } catch (error) {
    ...
    getHistoricalWeatherStats(city, date) {
        ...
        const historicalStats = {
            ...
            beijing: {
                春季: { // 3-5月
                    ...
                    rainProbability: 30,
                    windLevel: "微风",
                    tips: ["春季是北京最佳旅游季节", "温差较大，注意增减衣物", "可能有沙尘天气"]
                },
                夏季: { // 6-8月
                    avgTempHigh: 31,
                    avgTempLow: 21,
        ...
        if (historicalStats[normalizedCity] && historicalStats[normalizedCity][season]) {
            return {
                city: city,
                season: season,
    ...
    buildSuggestionsFromHistoricalData(historicalData, city, startDate, days) {
        const { avgTempHigh, avgTempLow, commonWeather, rainProbability, tips } = historicalData;
        ...
        return {
            ...
            duration: `${days}天`,
            forecast: this.generateHistoricalForecast(historicalData, startDate, days)
        };
    },
    ...
    generateHistoricalForecast(historicalData, startDate, days) {
        ...
        for (let i = 0; i < Math.min(days, 7); i++) {
            ...
            // 随机选择天气类型
            const weatherType = commonWeather[Math.floor(Math.random() * commonWeather.length)];
    ...
    generateSeasonalFallbackSuggestions(city, startDate, days) {
        ...
        const seasonalAdvice = {
            春季: {
                ...
                tips: ["春季气候宜人，适合户外活动", "注意昼夜温差", "可能有春雨，建议携带雨具"],
                commonWeather: "多云转晴"
            },
            夏季: {
                tempRange: "22℃~32℃",
                activities: ["室内博物馆", "购物中心", "水上活动", "夜间游览"],
                tips: ["夏季炎热，注意防暑降温", "多补充水分", "避免午后高温时段户外活动"],
                commonWeather: "晴转多云"
            },
            秋季: {
                tempRange: "12℃~25℃",
