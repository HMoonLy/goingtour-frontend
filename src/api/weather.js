...
export const weatherApi = {
            ...
            if (data && Array.isArray(data)) {
                ...
                data.forEach(city => {
                    if (city.中文名 && city.adcode) {
                        ...
                        ['市', '区', '县', '自治区', '自治州', '特别行政区'].forEach(suffix => {
                            if (cityName.endsWith(suffix)) {
                                const shortName = cityName.slice(0, cityName.length - suffix.length);
                ...
                console.log(`✅ 城市编码数据加载完成，共处理 ${processedCount}/${data.length} 条有效记录`);
                return true;
            } else {
    ...
    async getCityCode(cityName) {
        ...
        for (const name of namesToTry) {
            ...
            if (this.cityCodeMap[lowerName]) {
                console.log(`🗺️ 从缓存获取城市编码: ${name} -> ${this.cityCodeMap[lowerName]}`);
        ...
        for (const name of namesToTry) {
            const lowerName = name.toLowerCase();
            if (this.cityCodeMap[lowerName]) {
                console.log(`🗺️ 从本地数据获取城市编码: ${name} -> ${this.cityCodeMap[lowerName]}`);
                return this.cityCodeMap[lowerName];
            }
        }
            ...
            try {
                ...
                if (forecastResponse) {
                    ...
                    if (forecastResponse.data) {
                        console.log('天气预报API响应数据结构:', JSON.stringify(forecastResponse.data).substring(0, 200) + '...');
                    } else {
                ...
                if (forecastResponse.data.status !== '1') {
                    console.error('天气API错误响应详情:', JSON.stringify(forecastResponse.data));
                ...
                // 检查forecasts数组是否存在
                if (!forecastResponse.data.forecasts || forecastResponse.data.forecasts.length === 0) {
                    console.error('天气API返回格式不正确:', JSON.stringify(forecastResponse.data));
                ...
                return this.generateWeatherSuggestions(currentWeather, forecastData, startDate, days);
            } catch (error) {
    ...
    generateWeatherSuggestions(liveWeather, forecastWeather, startDate, days) {
        ...
        if (!liveWeather && forecastWeather && forecastWeather.casts && forecastWeather.casts.length > 0) {
        ...
        // 如果有天气预报，使用预报的温度范围
        if (forecastWeather && forecastWeather.casts && forecastWeather.casts.length > 0) {
        ...
        // 生成天气描述
        const weatherDesc = Array.from(weatherTypes).map(w => weatherTypeMap[w] || w).join('、');
        ...
        if (currentWeatherType.includes('雨') || currentWeatherType.includes('雪')) {
            // 根据雨量大小分类处理
            if (currentWeatherType.includes('小雨') || currentWeatherType.includes('小雪')) {
                activities.push('室内博物馆', '购物中心', '美食探索', '咖啡馆文化');
                tips.push('天气有小雨/雪，建议携带雨具或保暖衣物');
                avoid.push('避免在雨雪天气进行远距离户外活动');
            } else if (currentWeatherType.includes('中雨') || currentWeatherType.includes('阵雨') ||
                currentWeatherType.includes('雷阵雨') || currentWeatherType.includes('中雪')) {
                activities.push('室内博物馆', '购物中心', '当地特色餐厅', '文化展览');
                tips.push('天气有中雨/雪，出行请务必携带雨具或保暖衣物');
                avoid.push('不建议进行户外活动', '注意路面湿滑');
            } else if (currentWeatherType.includes('大雨') || currentWeatherType.includes('暴雨') ||
                currentWeatherType.includes('大雪') || currentWeatherType.includes('暴雪')) {
        ...
        if (forecastWeather && forecastWeather.casts && forecastWeather.casts.length > 0) {
            // 根据需要的天数截取预报数据
            const maxDays = Math.min(days, forecastWeather.casts.length);
