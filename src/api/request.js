            ...
            if (tokenExpiry) {
                ...
                if (Date.now() + fiveMinutes >= expiryTime) {
                    try {
                        ...
                        if (refreshToken) {
                            const refreshResponse = await refreshTokenRequest.post("/user/refresh-token", {
                                refreshToken: refreshToken
                            });

                            if (refreshResponse.data.code === 200) {
                                const newToken = refreshResponse.data.data.accessToken;
                                const newExpiry = Date.now() + refreshResponse.data.data.expiresIn;

                                localStorage.setItem("goingtour_token", newToken);
                                localStorage.setItem("goingtour_token_expiry", newExpiry.toString());
