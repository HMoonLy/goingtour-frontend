...
export function sanitizeHtml(html) {
    ...
    // 4. 过滤不允许的标签（简单实现）
    html = html.replace(/<(\/?)([\w\-]+)([^>]*)>/g, (match, slash, tagName, attrs) => {
        ...
        if (attrs && allowedAttrs.length > 0) {
            // 简单的属性过滤（可以进一步优化）
            const filteredAttrs = attrs.replace(/(\w+)\s*=\s*["']?([^"'>\s]+)["']?/g, (attrMatch, attrName, attrValue) => {
...
export function sanitizeMarkdownHtml(html) {
    ...
    // 比如确保链接是安全的
    safeHtml = safeHtml.replace(/<a\s+href\s*=\s*["']([^"']+)["']/gi, (match, href) => {
        // 只允许http/https链接
        if (href.match(/^https?:\/\//)) {
            return match;
        }
