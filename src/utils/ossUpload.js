...
class OSSUploader {
    ...
    validateFile(file, type = 'avatar') {
        ...
        if (file.size > config.maxSize) {
            result.valid = false;
            result.errors.push(`文件大小超过限制 (${Math.round(config.maxSize / 1024 / 1024)}MB)`);
        }

        // 检查文件类型
        if (!config.allowedTypes.includes(file.type)) {
            result.valid = false;
            result.errors.push(`不支持的文件格式，请选择 ${config.allowedTypes.join(', ')} 格式的文件`);
        }
    ...
    async uploadFile(file, type = 'avatar', options = {}) {
        try {
            ...
            // 生成文件名
            const fileName = config.folder + this.generateFileName(file.name, options.prefix || '');

            // 上传进度回调
            const progressCallback = options.onProgress || ((p) => {
