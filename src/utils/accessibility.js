...
export class KeyboardNavigationManager {
    ...
    handleArrowNavigation(e) {
        ...
        const list = currentElement.closest('[role="listbox"], [role="menu"], [role="list"]');
        if (!list) return;

        const items = Array.from(list.querySelectorAll('[role="option"], [role="menuitem"], [role="listitem"]'));
    ...
    handleEscape(e) {
        ...
        if (modal) {
            const closeButton = modal.querySelector('[data-dismiss], .el-dialog__close');
            if (closeButton) {
                closeButton.click();
            }
        }
     ...
     */
    isInList(element) {
        return element.closest('[role="listbox"], [role="menu"], [role="list"]') !== null;
    }
...
export function useScreenReader() {
    const announce = (message, priority) => screenReader.announce(message, priority);
    const announcePageChange = (title) => screenReader.announcePageChange(title);
    const announceLoading = (isLoading, message) => screenReader.announceLoading(isLoading, message);
