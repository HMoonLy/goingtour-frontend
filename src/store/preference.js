...
export const usePreferenceStore = defineStore('preference', () => {
  ...
  const mergedPreferences = computed(() => {
    ...
    return {
      ...
      socialPreference: tripPrefs.socialPreference || deriveSocialPreference(userPrefs.mbtiType) || 'mixed',

      // 拍照偏好：优先使用行程偏好，否则基于标签推导
      photoPreference: tripPrefs.photoPreference || derivePhotoPreference(userPrefs.selectedTags || []) || 'casual',
  ...
  const mapUserTagsToTripGoals = (tags) => {
    ...
    const mappedGoals = tags
      .map(tag => tagGoalMap[tag])
      .filter(Boolean)
