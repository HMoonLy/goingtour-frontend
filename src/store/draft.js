  ...
  const loadDraft = async (draftId) => {
    try {
      ...
      const parsedDraft = {
        ...
        baseForm: typeof draft.baseForm === 'string' ? JSON.parse(draft.baseForm) : (draft.baseForm || {}),
        preferenceForm: typeof draft.preferenceForm === 'string' ? JSON.parse(draft.preferenceForm) : (draft.preferenceForm || {}),
        selectedAttractions: typeof draft.selectedAttractions === 'string' ? JSON.parse(draft.selectedAttractions) : (draft.selectedAttractions || []),
        selectedRestaurants: typeof draft.selectedRestaurants === 'string' ? JSON.parse(draft.selectedRestaurants) : (draft.selectedRestaurants || []),
        extraRequirements: draft.extraRequirements || '',
        weatherSuggestion: typeof draft.weatherSuggestion === 'string' ? JSON.parse(draft.weatherSuggestion) : draft.weatherSuggestion
      }
