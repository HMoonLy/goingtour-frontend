...
export const useUserStore = defineStore('user', {
    ...
    getters: {
        ...
        // 获取用户手机号
        phone: (state) => state.currentUser?.phone,

        // 获取用户头像
        avatar: (state) => state.currentUser?.avatar || '/images/default-avatar.jpg',
        ...
        async updateUserPreferences(preferences) {
            ...
            try {
                ...
                if (response.data) {
                    this.userPreferences = {
                        ...this.userPreferences,
                        budget: preferences.dailyBudget || preferences.budget || this.userPreferences.budget
                    }

                    // 如果需要更新currentUser，可以重新获取用户信息
                    if (this.currentUser) {
                        this.currentUser.budget = preferences.dailyBudget || preferences.budget
                    }
                }
        ...
        restoreFromStorage() {
            try {
                ...
                if (userData) {
                    ...
                    this.userPreferences = parsedData.userPreferences || { tags: [], budget: 500 }
