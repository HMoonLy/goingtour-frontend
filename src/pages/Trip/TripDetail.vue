    ...
    <el-card class="trip-overview-card" shadow="hover">
      <div class="trip-header">
        <div class="trip-title-section">
          <h1 v-if="!isEditing" class="trip-title">{{ tripData.title }}</h1>
          <el-input 
            v-else 
      ...
      <div class="trip-info-grid">
        ...
        <div class="info-item">
          ...
          <div class="info-content">
            <h4>内容统计</h4>
            <p>{{ tripData.attractions?.length || 0 }}个景点 · {{ tripData.restaurants?.length || 0 }}家餐厅</p>
          </div>
        </div>
      </div>
    </el-card>
              ...
              <div class="activity-content">
                <div class="activity-header">
                  ...
                  <div class="activity-meta">
                    ...
                    <span v-if="activity.price" class="activity-price">¥{{ activity.price }}</span>
                  </div>
                </div>
                ...
                <div v-if="isEditing" class="activity-actions">
                  ...
                  <el-button size="small" type="danger" @click="removeActivity(dayIndex, actIndex)">
    ...
    <el-card class="preferences-card" shadow="hover">
      ...
      <div class="preferences-content">
        <div class="preference-group">
          <h4>行程强度</h4>
          <el-tag>{{ getIntensityText(tripData.preferences?.intensity) }}</el-tag>
        </div>
        
        <div class="preference-group" v-if="tripData.preferences?.experiences?.length > 0">
          <h4>特殊体验</h4>
          <div class="tags-row">
            <el-tag 
        ...
        <div class="preference-group" v-if="tripData.preferences?.transport?.length > 0">
          <h4>交通偏好</h4>
          <div class="tags-row">
            <el-tag 
        ...
        <div class="preference-group" v-if="tripData.preferences?.specialRequests">
          <h4>特殊要求</h4>
          <p class="special-requests">{{ tripData.preferences.specialRequests }}</p>
        </div>
      </div>
    </el-card>
  </div>
...
import { convertBackendTripToFrontend, convertFrontendTripToBackend } from '@/utils/tripDataConverter.js'
...
export default {
  ...
  setup() {
    ...
    const loadTripData = async () => {
      try {
        ...
        try {
          ...
          const response = await tripApi.getTripDetail(tripId.value, userStore.currentUser.id)
    ...
    const saveChanges = async () => {
      try {
        ...
        try {
          ...
          const response = await tripApi.updateTrip(tripId.value, userStore.currentUser.id, tripRequest)
    ...
    const formatDateRange = (createTime) => {
      ...
      try {
        const start = new Date(tripData.value.dateRange[0]).toLocaleDateString('zh-CN')
        const end = new Date(tripData.value.dateRange[1]).toLocaleDateString('zh-CN')
        return `${start} - ${end}`
      } catch {
        return '日期格式错误'
      }
    ...
    const saveAsTemplate = async () => {
      try {
        ...
        if (templateName) {
          ...
          const savedTemplates = JSON.parse(localStorage.getItem('tripTemplates') || '[]')
    ...
    const editActivity = async (dayIndex, actIndex) => {
      const activity = editedTrip.value.dailyPlan[dayIndex].activities[actIndex]
      ...
      try {
        ...
        const newName = lines.find(line => line.startsWith('名称：'))?.replace('名称：', '').trim()
        const newDesc = lines.find(line => line.startsWith('描述：'))?.replace('描述：', '').trim()
        const newTime = lines.find(line => line.startsWith('时间：'))?.replace('时间：', '').trim()

        if (newName) {
          editedTrip.value.dailyPlan[dayIndex].activities[actIndex].name = newName
        }
        if (newDesc) {
          editedTrip.value.dailyPlan[dayIndex].activities[actIndex].description = newDesc
        }
        if (newTime && /^\d{2}:\d{2}$/.test(newTime)) {
          editedTrip.value.dailyPlan[dayIndex].activities[actIndex].time = newTime
        }
    ...
    const replaceActivity = async (dayIndex, actIndex) => {
      const currentActivity = editedTrip.value.dailyPlan[dayIndex].activities[actIndex]
    ...
    // 替换景点
    const replaceWithAlternativeAttraction = async (dayIndex, actIndex, currentActivity) => {
      try {
        // 获取备选景点（排除已选的）
        ...
        const response = await fetch(`http://localhost:8080/api/attractions/city/${tripData.value.destination}`)
        ...
        if (response.ok) {
          ...
          const mockAlternatives = [
            { id: 'alt_attr_1', name: '替代景点A', description: '精彩的替代景点', price: '￥30' },
            { id: 'alt_attr_2', name: '替代景点B', description: '另一个不错的选择', price: '￥25' }
          ]
    ...
    // 替换餐厅
    const replaceWithAlternativeRestaurant = async (dayIndex, actIndex, currentActivity) => {
      try {
        // 获取备选餐厅（排除已选的）
        ...
        const response = await fetch(`http://localhost:8080/api/restaurants/city/${tripData.value.destination}`)
        ...
        if (response.ok) {
          ...
          // 使用模拟数据
          const mockAlternatives = [
            { id: 'alt_rest_1', name: '替代餐厅A', cuisine: '本地美食', price: '￥80/人' },
            { id: 'alt_rest_2', name: '替代餐厅B', cuisine: '特色料理', price: '￥60/人' }
          ]
    ...
    const addAttractionActivity = async (dayIndex) => {
      try {
        ...
        if (response.ok) {
          ...
          const { value: selectedIndex } = await ElMessageBox.prompt(
            '请选择要添加的景点（输入序号）：\n' + 
            availableAttractions.map((attr, i) => 
              `${i + 1}. ${attr.name} - ${attr.description || '精彩景点'}`
            ).join('\n'),
            '选择景点',
            {
    ...
    const addRestaurantActivity = async (dayIndex) => {
      try {
        ...
        if (response.ok) {
          ...
          const { value: selectedIndex } = await ElMessageBox.prompt(
            '请选择要添加的餐厅（输入序号）：\n' + 
            availableRestaurants.map((rest, i) => 
              `${i + 1}. ${rest.name} - ${rest.cuisine || '美食'}`
            ).join('\n'),
            '选择餐厅',
            {
    ...
    const addCustomActivity = async (dayIndex, defaultType = 'custom') => {
      try {
        ...
        // 解析用户输入
        const lines = formData.split('\n')
        const name = lines.find(line => line.startsWith('名称：'))?.replace('名称：', '').trim()
        const description = lines.find(line => line.startsWith('描述：'))?.replace('描述：', '').trim()
        const time = lines.find(line => line.startsWith('时间：'))?.replace('时间：', '').trim()
        const price = lines.find(line => line.startsWith('价格：'))?.replace('价格：', '').trim()
