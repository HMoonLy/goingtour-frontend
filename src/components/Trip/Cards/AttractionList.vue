      ...
      >
        ...
        <div class="recommendation-content-body">
          ...
          <div class="recommendation-rating rating-with-number">
            <el-rate
              :model-value="Number(attraction.rating) || 0"
              disabled
              text-color="#ff9900"
            />
            <span class="rating-value">{{ attraction.rating || '暂无' }}</span>
          </div>
          ...
          <div class="attraction-tags">
            ...
            <div
              v-if="getTags(attraction).length > 0"
              class="feature-tags"
            >
              <el-tag
                v-for="(tag, index) in getTags(attraction).slice(0, 3)"
                :key="index"
                size="small"
                effect="plain"
                type="info"
          ...
          <div class="add-to-plan">
            <el-button
              v-if="!isSelected(attraction)"
              size="small"
              plain
              @click="$emit('add', attraction)"
            >
    ...
    watch(() => props.sortBy, (newSort) => {
        ...
        if (newSort === 'rating') {
            sorted.sort((a, b) => {
             ...
             // 默认排序：如果是搜索结果，可能保持原样；如果是推荐，可能需要重新请求或者恢复顺序
             // 简单起见，如果切回默认，如果是推荐数据，可以重新loadData(true)来恢复API顺序，或者只是按ID或其他默认逻辑
             // 这里暂不处理复杂的恢复，或者重新加载
             if (!isSearchMode.value) {
                 // 如果是默认排序，且当前不是搜索模式，重新获取一下可能更稳妥，或者在本地存一份原始数据
                 // 为简单起见，这里触发重新加载
                 loadData(true);
                 return;
             }
