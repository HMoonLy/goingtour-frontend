...
export default {
  ...
  setup(props, { emit }) {
    ...
    // 提取标签辅助函数
    const getTags = (item) => extractHotelTags(item);
    ...
    const loadData = async (reset = false) => {
      ...
      try {
        ...
        const newItems = (response.pois || []).map(poi => ({
          ...
          photos: poi.photos || [],
          type: poi.type ? poi.type.split(";")[0] : "酒店",
